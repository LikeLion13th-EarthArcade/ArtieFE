...
export default function ExhibitionMiniCard({ ex }: ExhibitionMiniCardProps) {
    ...
    return (
        <div
            className="w-full h-full rounded-2xl border border-primary-300 shadow-[0_4px_10px_0_rgba(0,0,0,0.1)] p-4 flex flex-col cursor-pointer"
            onClick={handleCardClick}
        >
            <img src={ex.image} alt={ex.title} className="rounded-xl w-full h-[130px] object-cover" />
            <div className="mt-3 flex-1 flex flex-col">
                <h3 className="text-base w-full overflow-hidden text-ellipsis whitespace-nowrap">{ex.title}</h3>
